...
defmodule Mix.Tasks.Pleroma.Database do
  ...
  def run(["set_text_search_config", tsconfig]) do
    ...
    %{messages: msg} = Ecto.Adapters.SQL.query!(Pleroma.Repo, "ALTER DATABASE #{db} SET default_text_search_config = '#{tsconfig}';")
    if length(msg) > 0 do # non-exist config will not raise excpetion but only give >0 messages
