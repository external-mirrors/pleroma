...
defmodule Pleroma.Workers.RemoteRepliesFetcherWorker do
  ...
  def enqueue_for_object(
    ...
    if enabled?() and not Object.local?(object) and Visibility.public?(object) and
  ...
  def enqueue_for_reply_ancestors(%Object{} = object, depth) do
    if enabled?() and not Object.local?(object) and Visibility.public?(object) do
      object
      |> reply_ancestors(triggered_refresh_ancestor_depth(), depth)
      |> Enum.each(fn
        {%Object{data: %{"id" => object_id, "replies_collection" => collection_id}} = object,
         depth} ->
          if is_binary(collection_id) and not Object.local?(object) and Visibility.public?(object) and
  ...
  def perform(%Oban.Job{
    ...
    with {:parent, %Object{} = object} <- {:parent, Object.get_cached_by_ap_id(object_id)},
         ...
         {:public, true} <- {:public, Visibility.public?(object)},
  ...
  defp handle_fetch_error(reason) when reason in [:forbidden, :not_found], do: {:cancel, reason}
