  ...
  def fix_attachments(%{"attachment" => attachment} = object) when is_list(attachment) do
    attachments =
      Enum.map(attachment, fn data ->
        ...
        media_type =
          cond do
            is_map(url) && url =~ Regex.compile!(Pleroma.Constants.mime_regex()) ->
              url["mediaType"]

            is_bitstring(data["mediaType"]) && data["mediaType"] =~ Regex.compile!(Pleroma.Constants.mime_regex()) ->
              data["mediaType"]

            is_bitstring(data["mimeType"]) && data["mimeType"] =~ Regex.compile!(Pleroma.Constants.mime_regex()) ->
              data["mimeType"]
