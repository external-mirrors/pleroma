...
defmodule Pleroma.Web.AdminAPI.DomainControllerTest do
  ...
  describe "POST /api/pleroma/admin/domains" do
    ...
    test "create a domain the same as the endpoint host", %{conn: conn} do
      conn =
        conn
        |> put_req_header("content-type", "application/json")
        |> post("/api/pleroma/admin/domains", %{
          domain: String.upcase(Pleroma.Web.Endpoint.host())
        })

      %{"error" => "invalid_domain"} = json_response_and_validate_schema(conn, 400)
    end

    test "create a malformed domain", %{conn: conn} do
      conn =
        conn
        |> put_req_header("content-type", "application/json")
        |> post("/api/pleroma/admin/domains", %{domain: "not a domain"})

      assert %{"errors" => %{"domain" => _}} = json_response_and_validate_schema(conn, 400)
    end
  ...
  describe "DELETE /api/pleroma/admin/domains/:id" do
    ...
    test "refuses to delete a domain that still has accounts", %{conn: conn} do
      {:ok, %{id: domain_id}} = Domain.create(%{domain: "pleroma.mkljczk.pl", public: true})

      insert(:user, domain_id: domain_id)

      conn = delete(conn, "/api/pleroma/admin/domains/#{domain_id}")

      assert %{"errors" => %{"users" => _}} = json_response_and_validate_schema(conn, 400)
      assert length(Domain.list()) == 1
    end

    test "returns 404 for an unknown domain", %{conn: conn} do
      conn = delete(conn, "/api/pleroma/admin/domains/123456")

      assert json_response_and_validate_schema(conn, 404)
    end
  end

  describe "unknown domains" do
    test "PATCH returns 404", %{conn: conn} do
      conn =
        conn
        |> put_req_header("content-type", "application/json")
        |> patch("/api/pleroma/admin/domains/123456", %{public: true})
