...
* `processed_activity_days`: Days to keep remote Delete, Undo and Update activities. Their effect is applied on arrival; the stored copy only serves to recognise duplicate deliveries. An Update that a local user still has a notification for is kept. A copy that arrives after this time is processed again. Defaults to `7`; `nil` keeps them forever.
* `tombstone_days`: Days to keep the tombstones of deleted remote posts, counted from the deletion. While it exists, a tombstone stops the deleted post from being fetched again, for example when a reply or repeat of it arrives. Defaults to `30`; `nil` keeps them forever.
